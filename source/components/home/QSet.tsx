...
const QSet = ({ faq, i }: { faq: any; i: number }) => {
  ...
  return (
    <motion.div
      custom={i}
      variants={variant}
      className="border-b border-white/50 pb-6"
    >
      <div
        className="flex gap-6 items-center justify-between font-semibold cursor-pointer"
        onClick={() => setShowAnswer((prev) => !prev)}
      >
        <div>{faq.question}</div>
        <div
          className={
            "text-2xl duration-200 " + (showAnswer ? " rotate-[135deg]" : "")
          }
        >
          <FaPlus />
        </div>
