...
const WorkStep = ({ i, number, title, subtitle }: WorkStepProps) => {
  return (
    <motion.div
      custom={i}
      variants={variant}
      className="flex items-start ssm:items-center justify-start gap-2 sm:gap-6 flex-col ssm:flex-row "
    >
      <div className="w-14 h-14 bg-white/15 flex items-center justify-center rounded-full text-2xl font-bold">
        {number}
      </div>
      <div className="flex-1">
        <h3 className="font-playfair font-medium text-xl sm:text-2xl pb-1.5">
          {title}
        </h3>
